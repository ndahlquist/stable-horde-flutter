// Forked from https://github.com/pskink/matrix_gesture_detector
...
class _MatrixGestureDetectorState extends State<MatrixGestureDetector> {
  ...
  void onScaleStart(ScaleStartDetails details) {
    if (!widget.shouldTranslate) {
      // In drawing mode. Pass touch events to parent.
  ...
  void onScaleUpdate(ScaleUpdateDetails details) {
    if (!widget.shouldTranslate) {
      // In drawing mode. Pass touch events to parent.
      widget.onDrawUpdate?.call(details.localFocalPoint);
      return;
    }
    ...
    if (widget.shouldScale && details.scale != 1.0) {
      ...
      // Limit maximum zoom, since there are numerical stability issues.
      if (matrix.getMaxScaleOnAxis() > 60000) {
    ...
    // Scale this so that we're working in a unit coordinate space.
    // This is important, so that the transform matrix can be used
    // across devices with different screen resolutions.
    final matrixCopy = matrix.clone();

    matrixCopy.storage[12] /= context.size!.width;
    matrixCopy.storage[13] /= context.size!.height;

    widget.onMatrixUpdate(matrixCopy);
  }
